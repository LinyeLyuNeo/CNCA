...
def get_sd_models(opt):
    ...
    # sd_model = model.to(opt.diffusion_device)
...
def diffusion_inference(opt, model, sampler, adapter_features, append_to_context=None):
    ...
    )  # turn off torch.no_grad() to get the gradient

    x_samples = model.decode_first_stage(samples_latents)  # turn off torch.no_grad() to get the gradient
    x_samples = torch.clamp((x_samples + 1.0) / 2.0, min=0.0, max=1.0)

    return x_samples

def diffusion_inference_prompt_optim(opt, model, sampler, adapter_features,cond_prompt_optim, append_to_context=None):
    # # get text embedding
    # c = model.get_learned_conditioning([opt.prompt])
    # if opt.scale != 1.0:
    #     uc = model.get_learned_conditioning([opt.neg_prompt])
...
def diffusion_inference_latent_optim(opt, model, sampler, adapter_features, append_to_context=None):
    # # get text embedding
    # c = model.get_learned_conditioning([opt.prompt])
    # if opt.scale != 1.0:
    #     uc = model.get_learned_conditioning([opt.neg_prompt])
