...
from utils.general_ddp import bbox_iou_RAUCA_loss,xyxy2xywhn
from utils.torch_utils import is_parallel
...
class ComputeLoss:
    ...
    def __call__(self, p, targets):  # predictions, targets, model
        ...
        pxy=p_clone[:,:,:2]
        pwh=p_clone[:,:,2:4]
        pbox = torch.cat((pxy, pwh), 2)
        numberAnchor=p_clone.shape[1]
        pbox_squeeze=pbox
        pbox_squeeze=pbox_squeeze*(1/640)
        ...
        #c=torch.max(iou)
        p_clone_clone=p_clone.clone()
        b=torch.max(p_clone[:,:,7])
        p_clone_clone[:,:,5:]= p_clone[ :,:,5:]*iou
        #loss=torch.max(p_clone_clone[:,:,5:])
        loss=torch.max(p_clone_clone[:,:,7])
        loss_real=-torch.log(1-loss)
        return loss_real
        # device = targets.device
